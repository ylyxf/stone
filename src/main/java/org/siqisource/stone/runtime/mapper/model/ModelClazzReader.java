...
public class ModelClazzReader {
	 ...
	 * @return
	 */
	public static Model readModel(Class<?> mapperClazz, Class<?> modelClazz, DatabaseMetaData metaData, Dialect dialect,
			boolean isSingleKey) {
		// initial model
		...
		if (model.isSingleKey()) {
			...
			if (fieldMap.containsKey(propertyName)) {
				...
				throw new RuntimeException("  model " + model.getName() + " does not have a property point to table "
						+ model.getTableName() + "'s singleKey " + primaryColumn);
			}
